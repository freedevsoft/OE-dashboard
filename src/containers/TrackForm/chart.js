    ...
    if (item.accessed) {
      item.accessed.forEach(access => {
        ...
        if (!selectedAction || selectedAction === action) {
          // const uAction = action === 'Article Active' ? 'Active' : action === 'Article Inactive' ? 'Inactive' : action
          const uAction = action
          const index = data.findIndex(col => col.name === uAction)
          const articleName = directory[item._id]
          if (index >= 0) {
            data[index][articleName] = counter
          } else {
            data.push({ name: uAction, [articleName]: counter })
          }
        }
      })
    }
  ...
  return (
    <>
      <Chart height={400} data={dv} scale={cols} forceFit>
        ...
        />
        <Geom type="interval" position="article*count" size={15} color="name" adjust={[{ type: 'dodge', marginRatio: 0.1 / 32 }]} />
      </Chart>
    </>
  )
}
