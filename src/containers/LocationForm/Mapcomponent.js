...
const MapComponent = props => {
  const { zoom, zones, color, unit, coordinates, center, type, showCenterMark, yourPosition, onPointAdd, points } = props
  ...
  if (type === 'polygon') {
    ...
    if (points && points.length) {
      containers = points ? points.map((coordinate, index) => <Marker key={index} position={{ lat: coordinate.lat, lng: coordinate.lng }} />) : ''
    }
  } else if (type === 'radius') {
