      ...
      return (
        <Form.Item label={index === 0 ? label : ''} key={index} style={{ marginBottom: '10px' }}>
          <Row gutter={16}>
            ...
            <Col span={4}>
              <OZInput
                field={`${arrayField}[${index}].label`}
                initialValue={getDepthValue(initialValue[index], 'label')}
                getFieldDecorator={getFieldDecorator}
                placeholder="Label"
                required={false}
                style={{ marginBottom: '0px' }}
            ...
            <Col span={6}>
              {getFieldDecorator(`${arrayField}[${index}].iconURL`, { initialValue: getDepthValue(initialValue[index], 'iconURL') })(<TypesSelect collectionName="ImagesData" type="WebLinkImage" />)}
            </Col>
            <Col span={8}>
              <OZInput
