...
import { Button, Row, Col, Form, Icon, InputNumber, Select } from 'antd'
...
class OZPageLayout2 extends Component {
  static defaultProps = {
    extra: '',
    label: '',
    required: true,
      ...
      items.map((row, index) => {
        ...
        return (
          <Form.Item label={index === 0 ? label : ''} key={index} style={{ fontWeight: 'bold', marginBottom: '0' }}>
            <div className="oz-page-layout2-form-item">
                    ...
                    <Col span={column.ratio * 2} key={j}>
                      <Select
                        ...
                        onChange={layer => {
                          this.update(keysField, index, {
                            ...row,
                            columns: [...columns.slice(0, j), { ...columns[j], layer }, ...columns.slice(j + 1)],
                          })
                        }}
                        style={{
                          width: '90px',
