...
const FeaturedArticle = ({ myGeo, article, groupStyle }) => {
  ...
  switch (mMode) {
    case Modes.IMAGE:
      ...
      break
    case Modes.VIDEO:
      contentRender = <div className="oe-featured-layout-group-body-featured-bar-article-item-media-video">{videoList && videoList.length > 0 && <Player src={videoList[0].url} playsInline />}</div>
      break
    case Modes.LOC:
      contentRender = (
  ...
  const articleUpdatedAt = _.get(article, 'updatedAt') || null
  ...
  return (
    <div className="oe-featured-layout-group-body-featured-bar-article" key={articleID}>
      <div className="oe-featured-layout-group-body-featured-bar-article-item" style={articleOutline}>
        {mMode !== Modes.NONE && (
          <div className="oe-featured-layout-group-body-featured-bar-article-item-media">
            {contentRender}
            <div className="oe-featured-layout-group-body-featured-bar-article-item-media-selector" style={{ display: judgeMainBar > 1 ? 'flex' : 'none' }}>
              {bImageList && (
                <span>
              ...
              {bLocation && (
                <span>
                  <FontAwesomeIcon icon={faMapMarkerAlt} color="#1b95e0" onClick={() => setMode(Modes.LOC)} />
                </span>
              )}
            </div>
          </div>
        ...
        {(articleTitle || articleShortDescr || articleLongDescr) && (
          <div className="oe-featured-layout-group-body-featured-bar-article-item-text">
            <div className="oe-featured-layout-group-body-featured-bar-article-item-text-title">{articleTitle}</div>
            <div className="oe-featured-layout-group-body-featured-bar-article-item-text-short">{articleShortDescr}</div>
            <div className="oe-featured-layout-group-body-featured-bar-article-item-text-long">{articleLongDescr}</div>
          </div>
        )}
        <div className="oe-featured-layout-group-body-featured-bar-article-item-time">{`Updated: ${moment(articleUpdatedAt).format('MMM DD, hh:mm A')}`}</div>
      </div>
    </div>
  )
}
